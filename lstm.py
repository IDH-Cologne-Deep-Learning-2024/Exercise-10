...
# Inspect columns to ensure required fields are present
print("Columns in dataset:", df.columns)

# Adjust dropna to include only existing columns
df = df.dropna(subset=[col for col in ["Word", "POS", "Tag"] if col in df.columns])

# Ensure Word column is of string type
...
# Replace -1 with 0 for SparseCategoricalCrossentropy compatibility
...
for model_type in ['LSTM', 'BiLSTM', 'RNN']:
    ...
    num_pos_classes = np.max(y_train_pos) + 1  # Adjust based on maximum label value
    ...
    num_ne_classes = np.max(y_train_ne) + 1  # Adjust based on maximum label value
